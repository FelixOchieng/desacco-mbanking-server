 ...
 **/
public class ATMTransactionPoster implements Runnable {

    private final String CBS_SERVICE_NAME;
    private final int CBS_SERVICE;

    public ATMTransactionPoster() {
        this.CBS_SERVICE_NAME = "ATMTransactionPoster";
        this.CBS_SERVICE = 4; //POST ATM TRANSACTIONS
    }

    public static void start(long startDelay, long intervalPeriod) {
        ScheduledExecutorService executorService = new ScheduledThreadPoolExecutor(1);

        executorService.scheduleAtFixedRate(new ATMTransactionPoster(),
