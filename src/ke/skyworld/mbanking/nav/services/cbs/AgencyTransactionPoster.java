 ...
 **/
public class AgencyTransactionPoster implements Runnable {

    private final String CBS_SERVICE_NAME;
    private final int CBS_SERVICE;

    public AgencyTransactionPoster() {
        this.CBS_SERVICE_NAME = "AgencyTransactionPoster";
        this.CBS_SERVICE = 0; //POST AGENCY TRANSACTIONS
    }

    public static void start(long startDelay, long intervalPeriod) {
        ScheduledExecutorService executorService = new ScheduledThreadPoolExecutor(1);

        executorService.scheduleAtFixedRate(new AgencyTransactionPoster(),
    ...
    public void run() {
        try {
            ...
            CBSAgencyAPI.postMpesaTransaction("");
            instEnd = Instant.now();
